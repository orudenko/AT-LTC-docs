...
На маке нужно написать: brew update 

на убунту/mint: sudo apt update (он скорее всего спросит ваш пароль)
Практика 1.4: установка Clang, node.js и vscode

Мы поставим компилятор C (clang) и интерпретатор javascript (node.js)

на линуксе:sudo apt install clang
на маке: brew install --with-toolchain llvm (brew install node)
на виндовс: pacman -S clang
...
Если из командной строки, то sudo apt install путь_к_файлу или sudo dpkg -i путь к файлу.
